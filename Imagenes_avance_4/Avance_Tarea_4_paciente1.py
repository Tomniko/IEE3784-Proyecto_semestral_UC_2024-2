...
def segmentar_slice_automatica(ct_slice, initial_threshold, max_iter=5, window_size=2):
    """
...
def show_coronal_slices(img3d, img_shape, z_positions, pixel_spacing):
    ...
    def on_key(event):
        ...
        if event.key == 'x':
            ...
            img_suavizada = cv2.GaussianBlur(slice_img, (1,1), 5)
            mask = segmentar_slice_automatica(img_suavizada, initial_threshold=120)
            plt.figure()
            ...
            _, dist_bin  = cv2.threshold(dist, 140, 150, cv2.THRESH_BINARY)
...
def main():
    folder_path = 'Paciente 1 - 1'  # Ajusta esta ruta según corresponda
