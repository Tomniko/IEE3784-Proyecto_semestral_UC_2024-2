...
#def measure_heights_lr(mark, pixel_spacing):
    height_map = np.zeros_like(mark, dtype=np.float32)
    
    for region_label in np.unique(mark):
...
#def measure_heights_pca(mark, pixel_spacing):
    height_map_pca = np.zeros_like(mark, dtype=np.float32)
...
#def plot_height_maps(height_map, height_map_pca):
    ...
    # Gráfico de alturas en la dirección pies-cabeza
    ...
    # Gráfico de alturas en la dirección principal (PCA)
...
#def precompute_segmentation(img3d, img_shape, plane, threshold=112):
    """
    Realiza la segmentación para todas las slices de la imagen 3D y almacena los resultados.
    """
    segmented_slices = []
    for i in range(img_shape[plane]):
        slice_img = slice_det(img3d, i, plane)
        mask = segmentar_slice_automatica(slice_img, initial_threshold=threshold)
        contorno_final = segmentacion_mumford_shah(mask)
        filled_image = np.uint8(contorno_final.copy())
        
        # Operaciones de cierre morfológico
        kernel = np.ones((3, 3), np.uint8)
        dilated = cv2.dilate(filled_image, kernel, iterations=1)
        closed = cv2.erode(dilated, kernel, iterations=1)
        
        # Cálculo condicional para la extracción de vértebras
        ...
        # Normalización
        vertebrae_extraction_normalized = cv2.normalize(
            vertebrae_extraction, None, alpha=0, beta=255,
            norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U
        )
        segmented_slices.append(vertebrae_extraction_normalized)
...
def show_coronal_slices(img3d, img_shape, z_positions, pixel_spacing):
    ...
    def on_key(event):
        ...
        if event.key == 'x':
            ...
            mask = segmentar_slice_automatica(slice_img, initial_threshold=112)
            plt.figure()
...
#
        ...
        #    plt.show()
            ...
            # Calcular los mapas de altura
            #height_map = measure_heights_lr(mark, pixel_spacing)
            #height_map_pca = measure_heights_pca(mark, pixel_spacing)
            #
            ## Graficar las alturas
            #plot_height_maps(height_map, height_map_pca)
